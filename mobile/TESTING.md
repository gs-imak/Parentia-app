...
- Boutons disponibles :
  ...
  - `Tâche échéance proche` : envoie la notif pour une tâche email/photo avec deadline < 3 jours.
...
- Retards 09:00 : "N tâche(s) en retard." Tap → onglet Tâches filtré "retard" pour gérer individuellement.
- Urgente (immédiat) : déclenchée à la création email/photo si `isUrgentTask` (deadline ≤ J+3). Tap → détail tâche.
- Tâche échéance proche (immédiat) : déclenchée à la création email/photo si deadline < 3 jours. Tap → détail tâche.
...
- Exclusions immédiates :
  - Tâche urgente (source email/photo + deadline ≤ J+3).
...
- Urgent / Échéance proche → détail de la tâche.
...
- Échéance proche : seulement si source email/photo ET deadline < 3 jours.
